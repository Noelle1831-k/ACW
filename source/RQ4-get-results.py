...
def attack(strength, folder_path):
    command = []
    if strength == 0:
        command = [
            "ruff",
    ...
    result = subprocess.run(command_str, shell=True, text=True)
    if result.returncode == 0:
        print(f"Attack command executed successfully.")
    ...
    def folder_process(self):
        ...
        for folder_path in folder_list.folder_paths:
            if folder_path.startswith('G'):
                process(folder_path)
                attack(self.attack, folder_path)
        ...
        print(result)
        with open(f'output_attack_{self.attack}.json', 'w', encoding='utf-8') as f:
